...
# Órdenes estacionales SARIMA
# m = 12 porque la serie es mensual y presenta estacionalidad anual
seasonal_orders = [
    (0,0,0,12),
    (1,0,0,12),
    (0,1,0,12),
...
for order in orders:
    ...
    for seasonal_order in seasonal_orders:
        ...
        print(f"\nEvaluando SARIMAX {order} x {seasonal_order}")
        ...
        for train_idx, test_idx in tscv.split(X):
            ...
            try:
                ...
                print(f"Error con {order} x {seasonal_order}: {e}")
...
mae_sarimax_final = mean_absolute_error(
    y_test_final,
    y_pred_sarimax_final
)

rmse_sarimax_final = np.sqrt(
    mean_squared_error(
        y_test_final,
        y_pred_sarimax_final
    )
...
mape_sarimax_final = np.mean(
    np.abs((y_test_final - y_pred_sarimax_final) / y_test_final)
) * 100

print("\nSARIMAX FINAL RESULTS")
print("MAE:", mae_sarimax_final)
print("RMSE:", rmse_sarimax_final)
print("MAPE:", mape_sarimax_final)
# ============================================================
